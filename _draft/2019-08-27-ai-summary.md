...
1. Data science pipeline의 적용
-  모델링은 예측을 할 수있는 도구로 정보를 캡슐화하는 프로세스입니다. 주요 단계는 모델을 작성하고, 피팅하고, 검증하는 단계입니다.
흥미로운 문제제기(Ask an interesting question)
> 과학적 목표는? 
> 모든 데이터가 있다면 하고싶은 것은?
> 예측하고 싶은것은?
데이터 수집(Get the data)
...
> 어떤 데이터가 관련이 있나?
> 개인정보 이슈는 없나?
데이터 탐색(분석)(Explore the data)
> 시각화(plot the data.)
> 이상값이 있는지?
> 패턴이 있는지?
데이터 모델링(Model the data)
> 모델 생성
> 모델 학습(fit)
> 모델 검증
결과 공유 및 시각화(Communicate and Visualize the results)
> 무엇을 배웠나?
> 결과가 유의미 한가?
> 설명을 할 수 있나?

2. Google trend를 이용해 풀수 있는 문제 정의 및 접근방법
특정 검색 용어는 세계의 다양한 지역에서 다양한 언어로 전체 검색
볼륨을 기준으로 입력하는 빈도를 표시합니다.
사용자가 두 개 이상의 용어와 검색의 상대적인 검색량을 비교할 수 있습니다
...
5. Elo ranking 이해
- 엘로 평점시스템(ELO rating system) : 체스 선수들의 레이팅을 부여하기 위해 고안,  현재는 컴퓨터 게임의 래더 시스템에서 사용자 실력을 측정할 때 많이 쓰인다.
프로세스
1. 최초로 경기를 하는 사람은 1500점이 부여된다.
...
장점 : 서로간의 비교를 점수로 하기 때문에, 쉽고 간편하다, 서로 겨뤄보지 못한 참가자의 승률을 예측할 수 있다.

6. Power law 분포의 이해
- 멱법칙(Power-law Distribution) : 80:20 파레토 법칙과도 유사한 개념이다. 주변에서 찾아볼 수 있는 많은 분포들은 멱법칙(Power-law)을 따릅니다. 멱법칙을 따르는 경우 상위 몇 %가 전체의 대부분을 차지하고 나머지들이 긴 꼬리(long tail)을 형성하게 됩니다. (이는 빈부 격차와 같은 사회 경제적인 분야뿐만 아니라 음파의 주파수 별 진폭 분포처럼 자연 과학에서도 흔히 발견되는 분포입니다. 사람이 사용하는 언어의 단어 분포(지프의 법칙)나 소셜 네트워크의 중심성 계수 값도 이런 분포를 따릅니다.)

7. p-value의 이해
https://nittaku.tistory.com/448
http://blog.naver.com/PostView.nhn?blogId=jione322&logNo=220903024536
귀무가설 : 직접 검증의 대상이 되는 가설로 연구자가부정하고자 하는 가설입니다
대립가설 : 귀무가설에 반대되는 사실로 연구자가주장하고자 하는 가설입니다
즉, 통계적 결정에서 사실은 귀무가설이 진실임에도 불구하고 잘못된 데이터로 잘못 판단해 귀무가설을 기각할 오류를 제1종 오류라 하며, 제1종 오류를 범할 확률의 최대값을α라 표기하고 우리는 이α값을 유의수준이라 합니다. (일반적으로 유의수준은0.05로 많이 설정)
유의확률(p값)이란 현재 가지고 있는data에 대해 처음에 유의수준(α)을 얼마로 잡아야 가설검증 시 귀무가설을 기각할수 있는지를 계산한 값입니다.
따라서, 유의수준(α)을0.05로 정했을 때 계산된 유의확률(p값)이 0.05보다적게 나와야 1종 오류가 발생할 확률이 5/100 아래로 떨어지고 실험자는 비로소귀무가설을 기각하고 자신이 주장하고자 했던 대립가설을 채택할 수 있는 것입니다.
주의할 점은 유의수준과 p값은 가설이 맞고 틀림을 나타내는 지표가 아니라 확률을 표시하는 지표이다라는 것입니다.
특정 지표에 대해 p < 0.05라는 유의 확률을 나타낸 데이터를 해석할 때 “연구자가 주장(예상이95%,거의)맞다, 귀무가설이 잘못되었다.“ 라고 판단하는 것은 바람직하지 않습니다.
"귀무가설이 옳으나 귀무가설을 기각하고 대립가설을 연구자가 채택할 확률이 (제1종 오류를 범할 확률) 5%미만이다" 해당 데이터는 신뢰도가 높고 유의미한 데이터이다.

8. 효과적 데이터시각화 방법의 이해
- 그림의 가장 큰 가치는 예상하지 못한 것을 알 때입니다. John W. Tukey
터프의 디자인 원칙
좋은 / 나쁜 시각화를 구별하기 위해서는 디자인 미학과 데이터 표현하는 어휘(?)가 필요합니다.
 > 데이터 잉크의 비율을 최대화 : 색상을 조금 써라(?)
 > 거짓말 요인을 최소화 : 데이터와 그림의 크기 비율을 같게
 > chart junk 최소화 : 여분의 차원, 가치없는 색상, 과도한 그리드와 장식 삭제
 > 적절한 비율과 명확한 라벨
아름다운 데이터는 아름다운 시각화를받을 가치가 있습니다.
